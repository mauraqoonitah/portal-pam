	...
	if (sizeof($_POST)>0){
		...
		if ($front=="front"){
			...
			$baseurl  = 'backend/';
                ...
                //buat backup
                $folder_backup      = $basepath . 'main/generator_backup/' . $namamodule.'_'.date('YmdHis').'/';
                make_path($folder_backup);
                $folder_backup_module = $basepath . 'main/generator_backup/' . $namamodule.'_'.date('YmdHis').'/'.$namamodule.'/';
                make_path($folder_backup_module);
                $module_path_file   = $basepath . 'main/' . $namamodule.'.php';
                                
		if (file_exists($basepath . 'main/' . $namamodule.'.php')){
                    copy($module_path_file,$folder_backup.$namamodule.'.php');
                    unlink($module_path_file);
		}
    		...
    		ob_end_clean();
                ...
                if (!file_exists($namadir)) {
                    ...
                    xcopy($namadir, $folder_backup_module);
                    //mkdir($namadir,0777,true);
                }
