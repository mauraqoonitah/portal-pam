...
switch($do){
	...
	case 'list_jx':
		...
		break;
	case 'simpan_discount_jx';
		include_once($module.'/simpan_discount_jx.php');
		break;
	case 'tambah':
		tambah();
		break;
...
function approve(){
	global $module, $moduledb, $config;
			...
			if ($result) {
				//cek profile
				redirect('index.php?go='.$module,'Produk Berhasil di Approve','sukses');
			} else {
							$_SESSION['temp_form'] = $_POST;
                            $_SESSION['pesan']      = '<strong>Terjadi Error!</strong> Gagal menyimpan';
                            $_SESSION['pesan_mode'] = 'warning';
