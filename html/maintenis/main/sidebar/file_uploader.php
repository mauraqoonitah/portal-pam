...
//tampilkan form	
	...
	jQuery(function(){	
		...
		var inputtag = '<div class="input-group"><span class="input-group-btn"><span class="btn btn-file">Browse <input type="file" name="<?php echo $field; ?>" id="upload<?php echo $fieldid; ?>" multiple size="1" <?=$disabled?> /></span></span><input type="text" class="form-control" readonly></div>';
		...
		jQuery(document).on('change','#upload<?php echo $fieldid;?>',function(){
			...
			jQuery.ajax({
		            ...
		            success: function(responsetext) {
						//alert(responsetext);
						jQuery("#file_feed_<?php echo $fieldid;?>").html('<input type="hidden" name="upload_temp_<?php echo $field; ?>" id="upload_temp_<?php echo $fieldid;?>" value="'+responsetext+'"><a href="" class="hapus_temp_<?php echo $fieldid;?>"><img src="<?php echo $config['baseurl'].'images/delete2.png'; ?>" align="left" /></a>'+responsetext);
						jQuery('#file_progress_<?php echo $fieldid;?>').hide();
		...
		jQuery(document).on('click','.hapus_file_<?php echo $fieldid;?>',function(){
			if (confirm('Anda yakin ingin menghapus attachment ini?')) {
				...
				jQuery('#file_hapus_<?php echo $fieldid;?>').html('<input type="hidden" name="hapus_file_diattachment" value="'+valuenya+'"/>');	
				jQuery("#file_feed_<?php echo $fieldid;?>").html('');
				jQuery('#file_form_<?php echo $fieldid; ?>').html(inputtag);
				jQuery('#file_form_<?php echo $fieldid; ?>').show();		
			}
...
//echo '...'.$default_value;
			...
			<div class="input-group">
				<span class="input-group-btn">
					<span class="btn btn-file">
						Browse <input <?php echo $readonly; ?> type="file" name="<?php echo $field; ?>" id="upload<?php echo $fieldid; ?>" multiple size="1" <?=$disabled?> />
					</span>
				</span>
	...
	<div class="input-group">
		...
		<span id="file_feed_<?php echo $fieldid;?>">
			...
			<?php } ?>
			<a href="<?php echo $config['baseurl'].'attachment/'.$default_value;?>" target="_blank"><?php echo $default_value;?></a>
		</span>
	...
	?>
		<span id="file_progress_<?php echo $fieldid;?>" style="display: inline-block;">
