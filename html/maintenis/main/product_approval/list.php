...
<script language="javascript">	
	...
	$("body").tooltip({ selector: '[data-toggle=tooltip]' });
		...
		$(document).on('click','.btn_simpan',function(){
			var id = $(this).data('id');			
			var discount_craftynesia = $('#diskon_'+id).val();
			var kirim = 'discount_craftynesia='+discount_craftynesia+'&id='+id;
				$.ajax({		            
					url:'index.php?go=<?php echo $module;?>&do=simpan_discount_jx',
					type:'post',		            
					cache : false,		            
					data: kirim,		            
					success: 
					function(r) {		            	
						if (r.indexOf('sukses')>-1){
							alert('Diskon tersimpan');
						} else {
