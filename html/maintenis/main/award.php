...
$config['judul']    = 'Settings jasa banner';
...
function tambah(){
	...
	//cek apakah di post
		...
		if (sizeof($_POST)>0){
			...
			$result = doinsertdebug('award',$input);

			

			if ($result) {

				redirect('index.php?go=award','Data telah tersimpan');
...
function ubah(){
	...
	global $module, $config;
		...
		if (sizeof($_POST)>0){
			...
			$result = doupdatedebug("award",$input,"id='".$id."'");
			...
			if ($result) {
				...
				redirect('index.php?go=awards','Data telah tersimpan');
