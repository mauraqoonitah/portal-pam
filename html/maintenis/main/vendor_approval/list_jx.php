...
if (strlen($keyword)>0){
	...
	if (count($arrWhere)>0){
		...
		$where      = 'where';
        $strWhere   = 'role=5 and v.published=0';
	}
	
	$sql_from = '	from `'.$table_name.'` t 
					left join ec_vendor v on t.id=v.id_user
				' . $where.' '.
				    $strWhere.' '.
				    $order_by;
	...
	$field_select   = ' t.*, v.nama_vendor,v.nama_kontak_person,v.hp,v.hp2,v.telepon ';
            ...
            <th style="text-align: center; width:50px;">
                <div class="checkbox">
                    <label>
			            ...
			            <th class="<?php echo getClassSorting('email',$header_class); ?>">
                <a href="#" class="" data-name="email">
                    Email<span class="<?php echo getClassSortingIcon('email'); ?>"></span>
                </a>
            </th>
		                <?php  foreach ($header_table as $nama_field=>$judul_header) { ?>                <th class="hidden-xs <?php echo getClassSorting($nama_field,$header_class); ?>"><a href="#" class="" data-name="<?php echo $nama_field; ?>">
                    <?php
 echo $judul_header; 
...
?>                                        <li><a href="index.php?go=<?php echo $module; ?>&do=approve&id=<?php echo $row['id']; ?>" data-placement="top" data-toggle="tooltip" data-original-title="Approve"><i class="fa fa-fw fa-check"></i></a></li>
		                                ...
		                                <?php
 foreach ($header_table as $nama_field=>$judul_header) { 
?>								<td  class="hidden-xs" data-id="<?php echo $row['id']; ?>"><?php echo $row[$nama_field]; ?>							
                                <?php
 } 
?>		                             
                                </td>
