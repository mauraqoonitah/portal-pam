...
<?php
	$sql ='SELECT distinct COLUMN_NAME, COLUMN_COMMENT, data_type, CHARACTER_MAXIMUM_LENGTH FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = "'.$namatable.'" AND table_schema = "'.$config['database'].'"';
...
<?php
				...
				if (stripos($komennya,'combobox')!==false)
				{
					...
					if (!$adarelasi){
						...
						echo "<?php \$data".str_replace(' ','_',$schema['COLUMN_NAME'])." = doquery('select id,".$fieldshow." from ".$modelrelasi." order by id asc'); ?>\n";
