...
//$config['basepath']='/home/smkn1so1/public_html/';
$inputName='filedata';//表单文件域name
$attachDir=$config['basepath'] . 'images/' . 'upload';//上传文件保存路径，结尾不要带/
$dirType=1;//1:按天存入目录 2:按月存入目录 3:按扩展名存目录  建议使用按天存
$maxAttachSize=2097152;//最大上传大小，默认是2M
$upExt='pdf,txt,rar,zip,jpg,jpeg,gif,png,swf,wmv,avi,wma,mp3,mid';//上传扩展名
...
$tempPath .= '/'.date("YmdHis").mt_rand(10000,99999).'.tmp';
$localName='';
...
if($err==''){
	...
	if(preg_match('/'.str_replace(',','|',$upExt).'/i',$extension))
	{
		...
		else
		{
			...
			$urlPath    = $config['baseurl'] . 'images/upload/' . $attachSubDir . '/' . $newFilename;
			...
			else $msg="{'url':'".$urlPath."','localname':'".jsonString($localName)."','id':'1'}";//id参数固定不变，仅供演示，实际项目中可以是数据库ID
		}
	}
