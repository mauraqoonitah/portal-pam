...
<select name="<?php echo $field; ?>" id="<?php echo $fieldid; ?>" <?php if (strlen($class)>0) echo 'class="'.$class.'"'; ?> <?php if (strlen($style)>0) echo 'style="'.$style.'"'; ?> <?php echo $data_live_search;?> data-style="<?php echo $data_style;?>">
<?php
$dataselect = array();
if (count($option)>0){
	if (strlen($kosong)>0){
...
<select name="<?php echo $field; ?>" id="<?php echo $fieldid; ?>" <?php if (strlen($class)>0) echo 'class="'.$class.'"'; ?> <?php if (strlen($style)>0) echo 'style="'.$style.'"'; ?>>
<?php
if (count($option)>0){
...
?>
<?php if ($run_script=='false') {
    
} else { ?>
<script>
    $(function(){
       $('#<?php echo $fieldid; ?>').selectbox({
                                    effect: "fade",
                                    onChange: function (value, inst) {
                                            //alert('tes '+value+' '+inst.toSource());
                                    }
                            }); 
    });
</script>    
